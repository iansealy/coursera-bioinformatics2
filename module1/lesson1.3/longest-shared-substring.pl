...
# PODNAME: longest-shared-substring.pl
# ABSTRACT: Longest Shared Substring
...
# Default options
my $input_file = 'longest-shared-substring-sample-input.txt';
my ( $debug, $help, $man );
...
printf "%s\n",
  get_longest_shared_substring( $tree, $label, $colour, $text, 0, q{} );
...
# Get longest shared substring by DFS
sub get_longest_shared_substring {    ## no critic (ProhibitManyArgs)
    ## no critic (ProhibitReusedNames)
    ...
    my @seqs = ($seq);
    foreach my $next_node ( @{ $tree->{$node} } ) {
        if ( ref $tree->{$next_node} eq 'ARRAY' && $colour->{$next_node} == 0 )
        {
            my $next_seq = substr $text, $label->{$node}{$next_node}->[0],
              $label->{$node}{$next_node}->[1];
            push @seqs,
              get_longest_shared_substring( $tree, $label, $colour, $text,
...
This script solves the Longest Shared Substring Problem.

Input: Strings I<Text1> and I<Text2>.

Output: The longest substring that occurs in both I<Text1> and I<Text2>.

=head1 EXAMPLES

    perl longest-shared-substring.pl

    perl longest-shared-substring.pl \
        --input_file longest-shared-substring-extra-input.txt

    diff \
        <(perl longest-shared-substring.pl) \
        longest-shared-substring-sample-output.txt

    diff \
        <(perl longest-shared-substring.pl \
            --input_file longest-shared-substring-extra-input.txt) \
        longest-shared-substring-extra-output.txt

    perl longest-shared-substring.pl --input_file dataset_296_6.txt \
        > dataset_296_6_output.txt
