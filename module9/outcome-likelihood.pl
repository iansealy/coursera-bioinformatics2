...
# PODNAME: outcome-likelihood.pl
# ABSTRACT: Outcome Likelihood
...
# Default options
my $input_file = 'outcome-likelihood-sample-input.txt';
my ( $debug, $help, $man );
...
sub viterbi {
    ...
    foreach my $i ( 1 .. ( scalar @{$emissions} ) - 1 ) {
        for my $state1 ( @{$states} ) {
            ...
            $s->{$state1}[$i]       = $max;
            $forward->{$state1}[$i] = $sum;
        }
    }

    my $prob = 0;
    foreach my $state ( @{$states} ) {
        $prob += $forward->{$state}[-1];
    }
...
This script solves the Outcome Likelihood Problem.
...
Output: The probability Pr(I<x>) that the HMM emits I<x>.

=head1 EXAMPLES

    perl outcome-likelihood.pl

    perl outcome-likelihood.pl --input_file outcome-likelihood-extra-input.txt

    diff <(perl outcome-likelihood.pl) outcome-likelihood-sample-output.txt

    diff \
        <(perl outcome-likelihood.pl \
            --input_file outcome-likelihood-extra-input.txt) \
        outcome-likelihood-extra-output.txt

    perl outcome-likelihood.pl --input_file dataset_11594_8.txt \
        > dataset_11594_8_output.txt
